    ...
    pub async fn check_for_update(&mut self) -> Result<bool> {
        ...
        for app in Self::get_apps() {
            ...
            tasks.push(tokio::spawn(async move {
                {
                    let (result, version) = match app {
                        SupportedApp::Marzano | SupportedApp::Gouda => {
                            check_release_axo(app, &current_binary).await.unwrap()
                        }
                        _ => check_release_internal(app, &current_binary).await.unwrap(),
                    };
                    if result {
                        let mut found = found_updates.lock().await;
                        found.push((app, version.unwrap()));
                    }
                }
            }));
        }
...
pub async fn check_release_internal(
    ...
    if let Some(current_manifest) = current_binary {
        let result = manifest.release != current_manifest.release && manifest.release.is_some();
        Ok((result, manifest.version))
    } else {
        Ok((false, None))
    }
}
